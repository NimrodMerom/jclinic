...
export const BookingModal: React.FC<BookingModalProps> = ({ 
  ...
  }, [isOpen, initialDate, initialTime, initialRoomId, therapists]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 text-right" dir="rtl">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        {showSuccess ? (
          <div className="p-12 flex flex-col items-center justify-center space-y-4 animate-in zoom-in">
            <div className="w-16 h-16 bg-green-100 text-green-600 rounded-full flex items-center justify-center">
              <Check size={32} strokeWidth={3} />
            </div>
            <p className="text-xl font-bold text-gray-800">נשמר בהצלחה!</p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            {bookingType === 'one-off' && (
              <div className="flex gap-4 mb-4">
                <label className={`flex-1 cursor-pointer border rounded-lg p-3 flex items-center gap-2 transition-all ${oneOffSubtype === 'booking' ? 'border-indigo-500 bg-indigo-50 ring-1 ring-indigo-500' : 'border-gray-200 hover:bg-gray-50'}`}>
                  <input type="radio" name="subtype" checked={oneOffSubtype === 'booking'} onChange={() => setOneOffSubtype('booking')} className="hidden" />
                ...
                <label className={`flex-1 cursor-pointer border rounded-lg p-3 flex items-center gap-2 transition-all ${oneOffSubtype === 'absence' ? 'border-red-500 bg-red-50 ring-1 ring-red-500' : 'border-gray-200 hover:bg-gray-50'}`}>
                  <input type="radio" name="subtype" checked={oneOffSubtype === 'absence'} onChange={() => setOneOffSubtype('absence')} className="hidden" />
            ...
            <div>
              ...
              <select 
                ...
                required
              >
                <option value="" disabled>בחרו מטפל...</option>
                {therapists.map(t => (
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              {bookingType === 'one-off' ? (
                <div>
                  ...
                  <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-full rounded-md border-gray-300 border p-2 focus:ring-2 focus:ring-indigo-500 outline-none" required />
                </div>
              ) : (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">יום בשבוע</label>
                  <select value={dayOfWeek} onChange={(e) => setDayOfWeek(Number(e.target.value))} className="w-full rounded-md border-gray-300 border p-2 focus:ring-2 focus:ring-indigo-500 outline-none bg-white">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="time" value={startTime} onChange={(e) => setStartTime(e.target.value)} className="w-full rounded-md border-gray-300 border p-2 focus:ring-2 focus:ring-indigo-500 outline-none" required />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">סיום</label>
                <input type="time" value={endTime} onChange={(e) => setEndTime(e.target.value)} className="w-full rounded-md border-gray-300 border p-2 focus:ring-2 focus:ring-indigo-500 outline-none" required />
              </div>
            </div>
